...
import { Facebook, MailOpen, KeyRound, Phone } from 'lucide-react';
import { verificationTiers } from '../data/verificationTiers';

const LoginPage = ({ onLogin, showAlert, onRegister }) => {
    const [loginType, setLoginType] = useState('email'); // 'email', 'mobile', 'social', 'pin'
    ...
    const [selectedRole, setSelectedRole] = useState('fan'); // New state for role selection
    ...
    const handleSubmit = (e) => {
        ...
        if (forgotPasswordMode) {
            ...
            onRegister(selectedRole); // Pass the selected role to the parent App component
            showAlert(`Sign Up successful for: ${email || mobile || 'Social Account'} as a ${selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1)}`, 'success');
        } else {
    ...
    const renderFormFields = () => {
        ...
        return (
            <>
                ...
                {isSignUp && (
                    <div className="mt-4">
                        <label htmlFor="role-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Your Role:</label>
                        <select
                            id="role-select"
                            value={selectedRole}
